...
#library(fBasics)
#basicStats(german[,1]) # Common use of library
#germTable <- basicStats(german) # Common use of library
#head(germTable)

#germTable

#write.table(germTable, file = "germSummStats.csv", sep = ",")
...
credit.glm0<-glm(as.factor(response)~ ., family=binomial,data=german_train)
#
#summary(credit.glm0)
